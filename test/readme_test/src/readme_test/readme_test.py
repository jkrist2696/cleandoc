...
if __name__ == "__main__":
    ...
    os.mkdir(dirpath)
    scriptpath = os.path.join(maindir, "src", "readme_test", "readme_test.py")
    ...
    try:
        cd.cleandoc_all(
            dirpath, ignore=False, write=True, release="0.0.1"
        )  # Raises syntax warnings if ignore=False
    except SyntaxWarning as error:
        print(f"syntax warning: {error}")
    print("\n\n>>> Generating HTML Documents with Sphinx <<<\n")
    cd.gen_docs(dirpath, changed=True, release="0.0.1")
